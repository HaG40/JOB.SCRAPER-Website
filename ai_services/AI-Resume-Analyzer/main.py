...
- ควรใช้ Active Word เป็นภาษาเดียวกันกับเรซูเม่ เช่น Developed, Managed, Achieved, Increased, Reduced
...
- สายบัญชี: ระบุซอฟต์แวร์ (SAP, Express) ใบรับรอง (CPA) และมูลค่าบัญชีที่ดูแล
...
- หาก JD ระบุว่ารับผู้ไม่มีประสบการณ์ / ยินดีสอนงาน (Fresh Graduate / Trainee) → ทักษะที่เกี่ยวข้องถือเป็นข้อดีเสริม ไม่ใช่ข้อบังคับ และไม่ควรระบุว่าไม่เหมาะสมหากขาดทักษะบางอย่าง
...
2. ประสบการณ์ทำงาน (Work Experience)
   ...
   - จำนวนปีประสบการณ์รวมตรงตามที่ JD กำหนดหรือไม่
     ...
     → ถ้า JD รับ Fresh Graduate ให้ประเมินทักษะเป็นข้อดีเสริมแทน
...
3. ทักษะและการศึกษา (Skills & Education)
   - วุฒิการศึกษาตรงตามระดับที่ JD กำหนดหรือไม่
     ...
     → หากมีวุฒิการศกษาสูงกว่า ไม่ต้องระบุว่าไม่เหมาะสม แต่ให้พิจารณาว่าสาขาตรงกับที่ JD ต้องการหรือไม่
...
สิ่งที่ขาดหรือควรปรับ: (ข้อมูลหรือส่วนที่ไม่มีแต่ควรมี หรือมีแต่ไม่ควรมี รวมถึงอายุ/เพศที่ไม่ตรง JD ถ้ามี)

[EXPERIENCE]
ประเมิน: (สรุปจำนวนปีประสบการณ์รวมและความสอดคล้องกับที่ JD กำหนด)
⚠️ ข้อกำหนดที่ไม่ผ่าน: (ระบุถ้าประสบการณ์ไม่ถึงเกณฑ์ที่ JD กำหนด เช่น "JD ต้องการ 3 ปี แต่ผู้สมัครไม่มีประสบการณ์ → ไม่เหมาะสมกับตำแหน่งนี้")
...
- ผล: (ผ่าน / ⚠️ ไม่เหมาะสม — ระบุเหตุผล เช่น "JD ต้องการปริญญาโท แต่ผู้สมัครมีเพียงปริญญาตรี")

สาขาวิชาหรือความรู้เชิงวิชาการที่ตำแหน่งนี้คาดหวัง: (ระบุ)

Hard Skills ที่ JD ต้องการ:
- มีและตรง: (รายการ)
- มีบางส่วน: (รายการ)
...
- (ข้อที่เป็น Hard Disqualifier ให้ขึ้นต้นด้วย ⚠️ และระบุว่าเป็นเงื่อนไขที่ทำให้ไม่ผ่านการพิจารณาทันที)
...
เนื้อหาเรซูเม่/CV อยู่ในรูปภาพที่แนบมาด้านล่าง กรุณาอ่านและประเมินตามเกณฑ์ที่กำหนด"""
        ...
        return {
            "relevance":  trim_md_edges(relevance_match.group(1)) if relevance_match else "",
            "experience": trim_md_edges(exp_match.group(1))       if exp_match       else "",
            "skills":     trim_md_edges(skills_match.group(1))    if skills_match    else "",
            "advice":     trim_md_edges(advice_match.group(1))    if advice_match    else "",
        }
