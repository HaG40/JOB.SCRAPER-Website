...
func SingleScrapingJobbkk(keywrd string, page int, index int) (JobCard, error) {
	...
	err := c.Visit(scrapeURL)
	if err != nil {
		return JobCard{}, fmt.Errorf("failed to visit JobBKK: %w", err)
	}

	return jobbkkCards[index], nil
}

func DetailScrapingJobbkk(jobURL string) (string, error) {
	...
	c.OnError(func(_ *colly.Response, err error) {
		fmt.Printf("JobBKK detail scraping error: %v\n", err)
	})

	c.OnHTML("section.gridList", func(h *colly.HTMLElement) {
		selection := h.DOM
		jobDetail = strings.TrimSpace(selection.Find("section").Text())
		jobDetail = strings.TrimLeft(jobDetail, "แชร์\nเก็บงาน\nพิมพ์งาน\nร้องเรียน\nสมัครงาน\n  ")
		if idx := strings.Index(jobDetail, "การเดินทาง"); idx != -1 {
			jobDetail = jobDetail[:idx]
		}
		jobDetail = strings.ReplaceAll(jobDetail, "\t", "")
		jobDetail = strings.ReplaceAll(jobDetail, "\n\n", "")
		jobDetail = strings.ReplaceAll(jobDetail, "\n\n", "")
		jobDetail = strings.ReplaceAll(jobDetail, "\n\n", "")
		jobDetail = strings.ReplaceAll(jobDetail, "\n\n\n", "")
		...
		jobDetail = strings.ReplaceAll(jobDetail, "  ", "")
		jobDetail = strings.ReplaceAll(jobDetail, "  ", "")
		jobDetail = strings.TrimSpace(jobDetail)
	})

	err := c.Visit(jobURL)
	if err != nil {
		return "ไม่พบข้อมูลงาน", fmt.Errorf("failed to visit JobBKK job detail: %w", err)
	}
