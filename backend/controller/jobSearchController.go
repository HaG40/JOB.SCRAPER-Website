...
func JobSearchForRecommendation(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		randomIndex := rand.Intn(len(scraperFuncs))
...
func GetJobDetailHandler(w http.ResponseWriter, r *http.Request) {
	...
	switch source {
	case "jobbkk.com":
		jobDetail, err = scrapers.DetailScrapingJobbkk(jobURL)
	case "jobthai.com":
		jobDetail, err = scrapers.DetailScrapingJobthai(jobURL)
	default:
	...
	if err != nil {
		http.Error(w, "Failed to scrape job detail: "+err.Error(), http.StatusInternalServerError)
		return
	}

	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(map[string]string{"detail": jobDetail})
}
